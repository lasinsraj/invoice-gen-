...
const PreviewAddress: React.FC<PreviewAddressProps> = ({ title, addressData }) => {
  return (
    <div className="mb-6">
      ...
      <p className="text-gray-700 whitespace-pre-line break-words max-w-full overflow-hidden">{addressData.address}</p>
      {addressData.email && <p className="text-gray-700 break-all overflow-hidden">{addressData.email}</p>}
      {addressData.phone && <p className="text-gray-700 overflow-hidden">{addressData.phone}</p>}
    </div>
  );
};
